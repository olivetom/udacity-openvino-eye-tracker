...
class FacialLandMarksDetectionModel:
    ...
    def preprocess_output(self, outputs, facebox, image, print_flag=True, threshold=0.5):
        ...
        '''
        scaled_landmarks = outputs.reshape(1, 10)[0]
        ...
        if print_flag:
            for i in range(2):
                x = int(scaled_landmarks[i*2] * width)
                y = int(scaled_landmarks[i*2+1] * height)
                cv2.circle(image, (facebox[0]+x, facebox[1]+y), 30, (0, 255, i*255), 2)
        
        left_eye_point = [scaled_landmarks[0] * width, scaled_landmarks[1] * height]
        right_eye_point = [scaled_landmarks[2] * width, scaled_landmarks[3] * height]
